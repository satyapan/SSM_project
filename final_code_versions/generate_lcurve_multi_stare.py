...
outfile = open("lcurve_binned_stare_" + str(inttime) + "s.txt", "w")

output_MJD = []
output_events = []
stares = open("stares_"+str(inttime)+"s.txt", "w")
for filename in filenames:
	...
	for hduno in selectlist:
		...
		if len(temp) > 0:
			stares.write(str(min(temp))+" "+str(max(temp))+"\n")
		for m in temp:
			JD.append(m)
	...
	with open("lcurve_binned_stare_" + str(inttime) + "s.txt", 'a') as f:
...
staresref = loadtxt("stares_"+str(inttime)+"s.txt", float)
...
stares = zeros((len(init),2))
stares[:,0] = init
stares[:,1] = final

savetxt("stares_"+str(inttime)+"s.txt", stares)
